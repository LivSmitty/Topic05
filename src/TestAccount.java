/**
 * Created by scott_000 on 9/30/2016.
 */
public class TestAccount {
    public static void main(String[] args) {
        //create ID Array and Balance Array
        int idArray[] = new int[10];
        double balanceArray[] = new double[10];

        //fill arrays for ID and Balance
        for (int i = 0; i <= 9; i++) {
            idArray[i] = i;
            balanceArray[i] = 100.00;

        }


        //Account.mainMenu(balanceArray);


        //Create CheckingAccount object/call toString method
        int currentId = 8790;
        double checkingBalance = -300;

        CheckingAccount checkingAcc = new CheckingAccount(currentId, checkingBalance);
        System.out.println(checkingAcc.toString());


        //Create SavingsAccount object/call toString method
        double savingsBalance = 1000;
        SavingsAccount savingsAcc = new SavingsAccount(currentId, savingsBalance);

        System.out.println(savingsAcc.toString());
    }



}
